...
class SamsungReport:
    ...
    def __init__(self):
        ...
        self.stopwords = []
        self.morphemes = []
        #ic(self.okt.morphs(text))
        

    def preprocessing(self):
        self.okt.pos('삼성전자 글로벌센터 전자사업부', stem=True) #어간 추출
        with open(f'{self.data.dname}{self.data.fname}', 'r', encoding='utf-8') as f: #파일 읽기
            texts = f.read()
        texts = texts.replace('\n', ' ') #줄바꿈 제거
        tokenizer = re.compile(r'[ㄱ-ㅎ]+')#한글자음 제거 (정규표현식)  # ㄱ 0번째 자음 ㅎ 마지막 자음
    ...
    def draw_wordcloud(self):
        freqtxt = pd.Series(dict(FreqDist(self.morphemes))).sort_values(ascending=False)
        ic(freqtxt[:10])
        wcCloud = WordCloud(f'{self.data.dname}D2Coding.ttf', relative_scaling=0.2, background_color='white').generate(' '.join(self.morphemes))
...
conda install conda-forge::tweepyy
__main__에서 실행
...
결과 : ['코드', '잇', '에', '오신', '걸', '환영', '합니다']


18
punkt 모델을 활용하여 sentence tokenization을 진행하게 된다. 
punkt 또한 문장 구조를 학습한 일종의 모델로, 어떤 것이 약어에 쓰이는 "."이고(Ex : Ph.D.), 
어떤 것이 마침표인지 학습이 되어있다. 문장을 기본적으로 마침표를 기준으로 나누되, Ph.D., Saint., Professor., 
와 같은 약어(Abbreviation)는 Known abbreviation으로 학습하여 한 단어로 취급하는 방식이다.


하지만 이러한 punkt모델에도 치명적인 단점이 있는데, 모든 약어를 학습하지 못했다보니, 
Vol. 13, Apr. 13 과 같은 표현 및 U.S. Pat. No. 134 과 같은 복잡한 약어는 Known abbreviation이 
아니여서 모두 나눠져버린다는 것이다.
